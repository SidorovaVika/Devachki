...
class UserView(View):
    def dispatch_request(self,user_id):
        ...
        post = UserDepartment.query.filter(UserDepartment.user_id == cur_user_id).filter(UserDepartment.dismissal_date==None).first().post
        opportunities=[]
        for i in actions:
        ...
        dis_indic=False
        user_post=UserDepartment.query.filter(UserDepartment.user_id==user_id).filter(UserDepartment.dismissal_date==None).first().post
